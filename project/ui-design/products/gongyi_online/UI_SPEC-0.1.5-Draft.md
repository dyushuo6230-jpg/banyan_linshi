# UI_SPEC：众享源品 - 在线商城（应用级）· 0.1.5 Draft 历史

> 冻结副本。批准后的正文见同目录 `UI_SPEC.md`（0.1.5 Approved）。本文件不再修订。
...
| 替代文档 | 同目录 `UI_SPEC.md`（0.1.5 Approved）。本文是 0.1.5 Draft 冻结副本。 |

本文由第 30 节把**附近列表页**纳入 Partial。底栏 / 首页 / 分类沿用 0.1.4，不改。附近整页已定 PRD（M07、DEC-046、DEC-069、DEC-073、REQ-134、文件名删除项）写入第 4 节覆盖；与 PNG 打架时听这些口径，不升 locked 令牌。店详情 `details-f.vue` **不**列入本 Draft 换皮。禁止 `/v2` 业务双轨。下级 `devices/` 无独立 UI_SPEC，无端差异，不必标 Needs Review。

**本 Draft 不改代码、不换皮、不开 32A、不改 Contract。** `UI_SPEC.md` 仍为 0.1.4 Approved。未经第 29 节批准不得当像素验收。当前 Contract 仍禁止改附近。
...
- 版本 / 状态：0.1.5 **Draft**（Partial）。未经第 29 节批准不得升 Approved。本 Draft 不授权改 `go-uni-app-online`。
...
**附近列表（批准并同步 Contract、点名 32A 之后）：** 对第 4 节附近覆盖 + 金顶/搜索/灯片/店卡骨架。覆盖项（删除板块、CPS 轮播、距离/让利、写死类目）标豁免，不得挡验收，不得改代码去贴图。读序：当时的 `EFFECTIVE_UI_CONTRACT` → 批准后的本规范第 4 节 → `index-f.vue`。本 Draft **不得**当 32A 目标。分类页读序仍：Contract 0.1.4 → `UI_SPEC.md` 0.1.4 → `category.vue`。

本 Draft **不**自动开始换皮或 32A。
...
- CPS 点了是否跳第三方：业务听 DEC-046；现码本期只展示。批准本 Draft 不裁决跳转，32A 不得改。
