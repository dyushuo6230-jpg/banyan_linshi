# UI_SPEC：众享源品 - 在线商城（应用级）· 0.1.2 批准历史
...
| 批准人 | 会话授权用户 |
| 批准日期 | 2026-09-18 |
| 批准口令 | UI_SPEC_APPROVAL=UI-SPEC-gongyi_online@0.1.2 |
...
| 替代文档 | 同目录 `UI_SPEC-0.1.2-Draft.md`（Draft 历史）；`UI_SPEC-0.1.1.md`（上一批准冻结）；`UI_SPEC-0.1.0.md`。 |

本文已按第 29 节批准。覆盖 Partial：底栏、首页、分类。相对 0.1.1 只改首页横向分类选中的**实现与循环对照**：同盒 CSS 黑底 + 独立星 + CSS 三角，禁止组 A 整图拉伸。观感仍听 UI-DEC-004（黑底、青柠星、下三角、白字）。**本批准不改代码、不换皮、不改 Contract、不开 32A。** Contract 与循环须另开口令。禁止 `/v2` 业务双轨。不是 Baselined。
...
- 版本 / 状态：0.1.2 **Approved**（Partial）。不是 Baselined。本批准不授权改 `go-uni-app-online`（实现已在现码）。
...
本批准 **不**自动开始换皮、不开截图循环。第 32A 节须另点名。Contract 0.1.2 已同步三件套对照。
...
无新的高影响未决项需当场第 2A 节。首页选中实现口径已随本批准写入覆盖。
