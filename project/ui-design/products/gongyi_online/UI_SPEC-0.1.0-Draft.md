# UI_SPEC：众享源品 - 在线商城（应用级）· Draft 历史

> 冻结副本。批准后的正文见同目录 `UI_SPEC.md`（0.1.0 Approved）。本文件不再修订。
...
| 关联素材 | SRC-UI-003；MAT-GO-DOC-DESIGN；MAT-GO-TOK-DTCG |
| 关联决策 | UI-DEC-001、UI-DEC-002、UI-DEC-003、DEC-008、DEC-071、DEC-073、REQ-063 |
| 父级规范 | 无（`shared/UI_SPEC.md` 未启用，不继承后台/本地生活皮） |
| 代码真源 | `go-uni-app-online` |
| 替代文档 | 同路径 0.0.0 占位（Not Started）。本文是第一份 Draft，**未 Approved，不能当验收、不能换皮** |

本文由第 28A 节从 anydesign 产物映射。令牌与四条视觉争议已由 UI-DEC-003 确认。Partial 点名页：底栏、首页、分类。禁止 `/v2` 业务双轨。
...
- 版本 / 状态：0.1.0 **Draft**。未经第 29 节「确认并应用批准」不得升 Approved。
...
**locked（≥两页出现且已确认）：** `brand-gold` `#FFC31B`、`surface` `#FFFFFF`、`surface-page` `#F6F6F6`、`text-primary` `#222222`、`price` `#FD5800`、`accent-lime` `#A6F342`、`search-cta` `#111111`。底栏结构（五坑、中间缺口、橙红 AI）UI-DEC-003。

**禁止：** 令牌写入 `shared/`；`design.md` 复制进 `devices/`；业务路由 `/v2`；未 Approved 当像素验收；把《Tab Bar交互》当完整底栏。

## 5. 组件库和版本

无第三方设计系统。栈：UniApp Vue3、750rpx、现码零件。图标用现有 tabbar 资源；第四坑暂用发现图。无应用级 `ICON_MANIFEST.yaml`。

## 6. Design Token

真源：`materials/tokens/design-tokens.json`（由 `design.md` frontmatter 导出，禁止手改 JSON）。落地 rpx = 逻辑 px × 2。
...
字体：系统黑体 PingFang SC 等。列表/首页现价 20px/40rpx（`display`）；详情主价 28px/56rpx（`price-xl`）。圆角 pill 999px、卡片 lg 16px。阴影见 tokens。图标：侧边选中青柠填色；中间 AI 橙红标。
...
| 首页 | `/addon/shop/pages/indexPage/index` | 金顶 + 搜索 + 运营位 + 双列货 | `公益优选-商城首页定稿.png` |
| 分类 | `/addon/shop/pages/indexPage/category` | 金顶 + 搜索黑钮 + 左栏 + 右 3 列圆图 | `分类.png`（DEC-071） |

未点名有图页（列表、搜索、详情、我的、附近）令牌可复用，本 Draft **不**列入换皮范围。无 PNG 页 F0。
...
| chip | 选中浅金；分类横向胶囊 |
| category-rail | 左栏同高（不要甄选上新加高）；右 3 列圆图 |

## 10. 交互规范

触屏无悬停。底栏点击切 Tab，第四坑进购物车不是发现。分类左栏点一级，右侧展示该级下二级模块（DEC-071）。首页右上购物袋与底栏购物车都可进车。真扣款不要求（DEC-050）。
...
见 `design-a11y.md`。主按钮必须深字压金（9.91:1）。`text-muted` 不得用于长文。价签仅大号。白字只给黑搜索钮等深底。

## 14. 高保真要求

| 页 | 等级 | 说明 |
|---|---|---|
| 底栏、首页、分类 | F3（批准后） | 现 Draft 不验收。分类例外 DEC-071；第四坑不跟「发现」文案 |
...
**必须：** locked 令牌；底栏五坑+橙红 AI；第四坑购物车；金钮深字；首页右上购物袋；750rpx；接口 8035。

**推荐：** 模块间距 8/12/16；卡片靠灰底托白，少描边。

**禁止：** 青柠涂按钮/价签/顶栏；恢复发现业务；`/v2`；手改 tokens JSON；把源品色写入 shared。

## 16. 视觉验收标准

仅在本文 **Approved** 且点名换皮之后。视口 390×844。对照点名 PNG + 本规范覆盖项。建议第 32A 节循环，不是门禁。阈值：关键尺寸 ±1px；颜色对 locked 令牌。
...
无新的高影响未决项需当场第 2A 节；令牌争议已 UI-DEC-003。
