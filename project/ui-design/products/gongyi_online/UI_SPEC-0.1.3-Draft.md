# UI_SPEC：众享源品 - 在线商城（应用级）· 0.1.3 Draft 历史
...
| 替代文档 | 同目录 `UI_SPEC.md`（0.1.4 Approved）。本文是 0.1.3 Draft 冻结副本。 |

本文由第 30 节按 **DEC-071 v1.1.0** 把分类页图层、左栏箭头、二级代码金气泡写入覆盖。Partial 点名页仍是底栏、首页、分类。首页同盒三件套沿用 0.1.2，不改。禁止 `/v2` 业务双轨。下级 `devices/` 无独立 UI_SPEC，无端差异，不必标 Needs Review。

**本 Draft 不改代码、不换皮、不改 Contract、不开 32A。** `UI_SPEC.md` 仍为 0.1.2 Approved。循环在批准并同步 Contract 之前仍可能按旧 DEC-071 冻箭头/异形去修。

相对 0.1.2 只改分类页覆盖。不升级 locked `chip` 令牌。组 B 整图只作观感参考，禁止拉伸。组 C/D 箭头启用。
...
- 版本 / 状态：0.1.3 **Draft**（Partial）。未经第 29 节批准不得升 Approved。本 Draft 不授权改 `go-uni-app-online`。
...
本 Draft **不**自动开始换皮、不开截图循环。第 32A 节须在第 29 节批准并同步 Contract 之后另点名。
...
无新的高影响未决项需当场第 2A 节。分类页覆盖已写入本 Draft。

批准本 Draft 须用第 29 节原句：`确认并应用批准：UI_SPEC_APPROVAL=UI-SPEC-gongyi_online@0.1.3`
