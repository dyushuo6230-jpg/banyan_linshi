# UI_SPEC：众享源品 - 在线商城（应用级）· 0.1.4 批准历史

> 冻结副本。当前批准真源见同目录 `UI_SPEC.md`（0.1.5 Approved）。本文件不再修订。
...
| 批准人 | 会话授权用户 |
| 批准日期 | 2026-09-19 |
| 批准口令 | UI_SPEC_APPROVAL=UI-SPEC-gongyi_online@0.1.4 |
...
| 替代文档 | 同目录 `UI_SPEC.md`（0.1.5 Approved 当前真源）。本文是 0.1.4 批准冻结。`UI_SPEC-0.1.4-Draft.md` 为对应 Draft 历史。 |

本文已按第 29 节批准。覆盖 Partial：底栏、首页、分类。相对 0.1.3：分类页听 `category.vue` 现状（外底板 UI-DEC-005、左栏选中/grid/全部分类 UI-DEC-006、DEC-071 v1.3.0）。首页同盒三件套沿用 0.1.3。**本批准不改代码、不换皮、不开 32A。** 禁止 `/v2` 业务双轨。不是 Baselined。
...
- 版本 / 状态：0.1.4 **Approved**（Partial）。不是 Baselined。本批准不授权改 `go-uni-app-online`。
...
**禁止：** 令牌写入 `shared/`；`design.md` 复制进 `devices/`；业务路由 `/v2`；未 Approved 当像素验收；把《Tab Bar交互》当完整底栏；把组 B 整图当选中底拉伸；把组 A 整图当选中底拉伸；编造树上没有的一级；借 32A 按已冻结的 0.1.3 或《分类.png》改 `category.vue`。
...
分类页 32A 读序：`EFFECTIVE_UI_CONTRACT` 0.1.4 → 本规范第 4 节 → `category.vue`。与《分类.png》或已冻结的 0.1.3 冲突时以现码为准，该差异不得写入挡验收 DIFF，不得改代码。左栏须验两态。本批准 **不**自动开始换皮或 32A。
...
无新的高影响未决项需当场第 2A 节。B/E/F 及竖条禁切图、三级白卡 15rpx、左栏间距 20rpx、顶底淡入现码色、二级槽尺寸、全部分类 overlay/`520rpx`/深字选中，已按 `category.vue` 写入本规范。
