# UI_SPEC：众享源品 - 在线商城（应用级）· 0.1.1 批准历史
...
| 批准人 | 会话授权用户 |
| 批准日期 | 2026-09-18 |
| 批准口令 | UI_SPEC_APPROVAL=UI-SPEC-gongyi_online@0.1.1 |
...
| 替代文档 | 同目录 `UI_SPEC-0.1.1-Draft.md`（Draft 历史）；`UI_SPEC-0.1.0.md`（上一批准冻结）。 |

本文已按第 29 节批准。覆盖 Partial：底栏、首页、分类。相对 0.1.0 增加首页横向分类选中页面覆盖（组 A）与分类左栏顶底两态。**本批准不改代码、不换皮、不开 32A。** 落地须另开口令（Contract / 第 32A 节）。禁止 `/v2` 业务双轨。不是 Baselined。
...
- 版本 / 状态：0.1.1 **Approved**（Partial）。不是 Baselined。本批准不授权改 `go-uni-app-online`。
...
**必须：** locked 令牌；底栏五坑+橙红 AI；第四坑购物车；金钮深字；首页右上购物袋；750rpx；接口 8035。首页横向分类选中听组 A；分类左栏顶底两态。
...
本批准 **不**自动开始换皮或截图循环。Contract 与第 32A 节须另点名。
